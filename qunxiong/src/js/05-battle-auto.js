...
function stratChance(u,t){return clamp(0.35+(u.int-t.int)/100,0.05,0.9);}
function duelChance(u,t){return clamp(0.5+(u.war-t.war)/30,0.05,0.95);}
function confuse(t){t.conf=t.acted?1:2;}
function act(B,u,o){
 if(u.dead)return;
 ...
 const charge=()=>{ev.type='charge';const d=hurt(B,t,ubase(B,u)*(0.6+u.war/250)*1.25,true);const c=t.dead?0:hurt(B,u,ubase(B,t)*(0.6+t.war/250)*0.45,false);bl(B,`${u.name}隊突擊${t.name}隊，殲敵 ${fmt(d)}，自損 ${fmt(c)}`);};
 switch(o.cmd){
  case 'volley':{ev.type='volley';const d=hurt(B,t,ubase(B,u)*0.85,true);bl(B,`${u.name}隊向${t.name}隊齊射，殲敵 ${fmt(d)}`);break;}
  case 'strat':{
   ev.type='strat';ev.label=nm;
   if(Math.random()<stratChance(u,t)){ev.ok=true;const d=hurt(B,t,ubase(B,u)*(0.5+u.int/100)*1.3,false);confuse(t);bl(B,`${u.name}對${t.name}施${nm}成功，殲敵 ${fmt(d)}，敵隊陷入混亂`,'hit');}
   else if(t.int>u.int+10&&Math.random()<0.4){ev.backfire=true;confuse(u);bl(B,`${u.name}的${nm}被${t.name}識破，反陷混亂`,'bad');}
  ...
  case 'guard':{ev.type='guard';ev.to=null;B.morale[u.side]=clamp(B.morale[u.side]+3,0,100);bl(B,`${u.name}隊堅守陣地`);break;}
  case 'siege':{
   if(u.side!=='a'){charge();break;}
   ev.type='siege';ev.to=null;
   const w=Math.round(ubase(B,u)*0.18);B.wall=Math.max(0,B.wall-w);ev.wall=w;
...
function runRound(B,ords){
 ...
 if(!B.over)['a','d'].forEach(s=>{B.morale[s]=Math.round(clamp(B.morale[s]-B.loss[s]/B.start[s]*120+(B.loss[s]<B.loss[otherSide(s)]?2:0),0,100));});
...
function autoResolve(B){let g=0;while(!B.over&&g++<20)runRound(B,Object.assign(aiOrders(B,'a'),aiOrders(B,'d')));}
...
function finishBattle0(B){
 ...
 const sum=s=>alive(B,s).reduce((a,u)=>a+u.troops,0);
...
function checkFactions(){
 for(const f of Object.values(S.factions)){
  ...
  if(S.officers[f.lord].fac!==f.id){
   ...
   const cand=heir||S.officers.filter(o=>o.fac===f.id).sort((a,b)=>b.cha-a.cha)[0];
   if(cand){f.lord=cand.id;log(`${cand.name}繼承${f.name}勢力`,f.id===S.player?'bad':'');}
