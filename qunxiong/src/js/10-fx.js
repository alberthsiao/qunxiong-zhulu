...
async function playEv(B,ev){
 ...
 switch(ev.type){
  case 'charge':{if(from&&to){const a=hxy(...fu.pos),tu=B.units.find(x=>x.id===ev.to),b=hxy(...tu.pos);tween(from,[a,{x:a.x+(b.x-a.x)*.45,y:a.y+(b.y-a.y)*.45},a],200);}await fxWait(220);showHits(ev);await fxWait(420);break;}
  case 'volley':fly(from,to,'arrow',6,18);await fxWait(540);showHits(ev);await fxWait(250);break;
  case 'strat':floatText(from,ev.label,'strat',-26);await fxWait(300);
  ...
  case 'duel':{banner('陣前單挑','duel');await fxWait(700);
  ...
  case 'siege':{const g=gateEl(ev.gate);fly(from,g,'stone',3,50);await fxWait(500);
