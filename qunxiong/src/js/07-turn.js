...
function economy(){
 ...
 for(const c of Object.values(S.cities)){
  if(!c.owner)continue;
  const pf=0.7+(c.ppl??60)/200;c.gold+=Math.round((c.trade*0.3+c.pop/5000)*pf);
  ...
  if(S.month===7){const h=Math.round((c.farm*12+c.pop/50)*pf);c.food+=h;if(c.owner===S.player)harvest+=h;}
  ...
  c.pop=Math.round(Math.min(c.popMax,c.pop*(1+0.004*((c.ppl??60)-40)/60)+(c.farm+c.trade)/10));
 }
 if(harvest)log(`秋收，我軍各城共收糧 ${fmt(harvest)}`,'good');
}
function lifeCycle(){
 S.officers.forEach(o=>{const F=S.factions[o.fac];if(!F||F.lord===o.id)return;const L=S.officers[F.lord];if(L.cha>=85&&Math.random()<0.3)o.loy=Math.min(100,(o.loy||70)+1);else if(L.cha<60&&Math.random()<0.3)o.loy=Math.max(30,(o.loy||70)-1);});
 if(S.month===1)S.officers.forEach(o=>{if(o.fac==='unborn'&&o.appear<=S.year){o.fac=null;o.city=o.home;o.found=false;}});
...
function endTurn(){
 if(!S||!S.player||S.over)return;
 ui.mode=null;S.incoming=[];
 citiesOf(S.player).filter(c=>c.auto).forEach(c=>{const n0=S.log.length;autoDomestic(c,S.player);});
 for(const f of shuffle(Object.keys(S.factions))){if(f===S.player||!S.factions[f].alive)continue;aiTurn(f);}
 runIncoming(()=>{
  ...
  lifeCycle();
  runEvents();
  syncItems();Object.keys(S.factions).forEach(f=>{if(f!==S.player&&S.factions[f].alive)aiEquip(f);});
  aiDiplomacy();
  checkTitles();
  ...
  render();
  showEvents(()=>showProposals(()=>processCaptives(()=>render())));
 });
}
...
function checkEnd(){
 ...
 if(citiesOf(S.player).length===Object.keys(S.cities).length){S.over=true;render();modal('天下一統',endingHTML(true),[{label:'重新開始',primary:true,fn:showStart},{label:'留在地圖'}]);return true;}
 return false;
}
