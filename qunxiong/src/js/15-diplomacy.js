...
function drawDip(){
 ...
 h+=`<p class="hint">交涉成敗取決於使者的政治與魅力、雙方友好度、兵力對比，以及是否面對共同的強敵。提議同盟或停戰需備禮 100 金，金錢從使者所在城支出。盟友與停戰對象不會攻打你；你若出兵攻打他們，就會破棄盟約，其他勢力對你的友好度也會下降。</p>`;
...
$('#modal').addEventListener('click',e=>{const b=e.target.closest('[data-dp]');if(!b)return;const f=b.dataset.f,k=b.dataset.dp;
 ...
 const r=doDip(env,f,k,arg);log(r.msg,r.cls);toast(r.msg);drawDip();});
