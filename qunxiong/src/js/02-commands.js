...
function maxRecruit(c){return Math.max(0,Math.min(5000,Math.floor(c.gold/0.15/500)*500,Math.floor(c.pop*0.1/500)*500));}
function doRecruit(o,c,amt){const cost=Math.round(amt*0.15);if(amt<500||c.gold<cost)return null;const n=Math.round(amt*(1+o.cha/500));c.gold-=cost;c.pop-=n;c.train=Math.round((c.troops*c.train+n*40)/(c.troops+n));c.troops+=n;o.done=true;c.ppl=Math.max(0,(c.ppl??60)-Math.round(n/500));gainExp(o,'cha',10);return{msg:`${o.name}於${c.name}徵兵 ${fmt(n)} 人，花費 ${cost} 金，民忠略降`};}
...
function recruitChance(o,f){return clamp(0.15+(o.cha*0.4+lordOf(f).cha*0.6-60)/70,0.1,0.9);}
function doSearch(o,c,f){
 ...
 if(hid.length&&Math.random()<0.3+o.int/250){const t=hid[0];if(f===S.player)t.found=true;
  if(Math.random()<recruitChance(o,f)){t.fac=f;t.done=true;t.loy=75;return{msg:`${o.name}在${c.name}訪得${t.name}，${t.name}願意出仕`,cls:'good'};}
...
function persuadeChance(o,t,f){return clamp(recruitChance(o,f)-(t.city!==o.city?0.1:0),0.05,0.9);}
function doPersuade(o,t,f){o.done=true;gainExp(o,'cha',10);const far=t.city!==o.city;if(Math.random()<persuadeChance(o,t,f)){t.fac=f;t.done=true;t.loy=75;t.city=o.city;return{msg:`${t.name}接受${o.name}的邀請${far?'，前來'+o.city:''}，加入麾下`,cls:'good'};}return{msg:`${o.name}${far?'遣使':''}拜訪${t.name}，遭到婉拒`};}
function doMove(src,dst,offs,n){offs.forEach(o=>{o.city=dst.name;o.done=true;});if(n>0){dst.train=Math.round((dst.troops*dst.train+n*src.train)/(dst.troops+n));dst.troops+=n;src.troops-=n;}}
