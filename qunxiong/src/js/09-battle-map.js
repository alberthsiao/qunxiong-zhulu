...
const RIVER_CITIES=['建業','柴桑','江夏','江陵','吳','會稽','襄陽','壽春','下邳','濮陽','平原','洛陽','長沙'];
const MOUNT_CITIES=['漢中','梓潼','成都','永安','江州','天水','武威','晉陽','長安','宛'];
const hk=(c,r)=>c+','+r;
...
function initMap(B){
 const R=seeded(B.city),T={};
 for(let r=0;r<MH;r++)for(let c=0;c<MW;c++)T[hk(c,r)]='plain';
 const blob=(type,n,len)=>{for(let i=0;i<n;i++){let c=1+Math.floor(R()*7),r=Math.floor(R()*MH);for(let j=0;j<len;j++){if(T[hk(c,r)]==='plain')T[hk(c,r)]=type;const nb=nbrs(c,r);[c,r]=nb[Math.floor(R()*nb.length)];}}};
 ...
 if(riv||R()<0.35){let c=3+Math.floor(R()*3);for(let r=0;r<MH;r++){T[hk(c,r)]='river';if(R()<0.5)c=clamp(c+(R()<0.5?-1:1),2,6);}}
 const core=[10,4];const ring2=[],ring1=[];
 ...
 const west=[core[0]-2,core[1]];
 const nw=ring2.filter(p=>p[1]===core[1]-2).sort((a,b)=>a[0]-b[0])[0];
 const sw=ring2.filter(p=>p[1]===core[1]+2).sort((a,b)=>a[0]-b[0])[0];
 B.gates={};[west,nw,sw].forEach(g=>{const k=hk(...g);T[k]=B.wall>0?'gate':'breach';B.gates[k]=Math.round(B.wall);});
 const spawnA=[[0,4],[1,3],[1,5],[0,2],[0,6]];spawnA.forEach(p=>{T[hk(...p)]='plain';nbrs(...p).forEach(q=>{if(T[hk(...q)]==='mount')T[hk(...q)]='hill';});});
 B.T=T;B.core=core;
 B.units.filter(u=>u.side==='a').forEach((u,i)=>u.pos=spawnA[i]);
 const inside=[core,...ring1.sort((a,b)=>hxy(...a).x-hxy(...b).x)];
 B.units.filter(u=>u.side==='d').forEach((u,i)=>u.pos=inside[i]);
 B.units.forEach(u=>{u.mv=UT[u.type].mv+(u.off!=null&&itemsOf(S.officers[u.off]).some(i=>i.type==='名馬')?1:0);u.done=false;u.moved=false;u.guard=false;u.conf=0;});
...
function reach(B,u){
 ...
 while(q.length){q.sort((a,b)=>a[0]-b[0]);const[cost,[c,r]]=q.shift();const k=hk(c,r);if(all[k].cost<cost)continue;
  ...
  for(const[a,b]of nbrs(c,r)){const tt=B.T[hk(a,b)];const tc=TER[tt].cost+(u.type==='騎'&&(tt==='forest'||tt==='hill')?1:0);if(tc>=99)continue;const x=unitAt(B,a,b);if(x&&x.side!==u.side)continue;const nc=cost+tc;if(nc>u.mv)continue;const nk=hk(a,b);if(!all[nk]||all[nk].cost>nc){all[nk]={cost:nc,prev:k};q.push([nc,[a,b]]);}}}
...
function tdef(B,u){const t=B.T[hk(...u.pos)];if(t==='city'||t==='core')return 1/(1+B.wall/1000);return TER[t].def;}
function targetsFor(B,u,cmd){
 ...
 if(cmd==='volley')return foes.filter(f=>d(f)<=(u.type==='弓'?3:2));
...
function mapAct(B,u,cmd,tid,opts){
 ...
 if(cmd==='charge'){const d=hurt(B,t,ubase(B,u)*(0.6+u.war/250)*1.25*tdef(B,t)*tm(u,t)*(u.type==='騎'?1.15:u.type==='弓'?0.85:1),false);const c=t.dead?0:hurt(B,u,ubase(B,t)*(0.6+t.war/250)*0.45*tdef(B,u)*tm(t,u)*(t.type==='槍'?1.2:1),false);gainExp(O,'war',8);gainExp(O,'lea',4);bl(B,`${u.name}隊突擊${t.name}隊，殲敵 ${fmt(d)}，自損 ${fmt(c)}`);}
 else if(cmd==='volley'){const d=hurt(B,t,ubase(B,u)*0.85*tdef(B,t)*tm(u,t)*(u.type==='弓'?1.25:u.type==='騎'?0.7:1),false);gainExp(O,'lea',6);bl(B,`${u.name}隊向${t.name}隊齊射，殲敵 ${fmt(d)}`);}
 else if(cmd==='strat'){ev.label=nm;gainExp(O,'int',8);
  if(Math.random()<stratChance(u,t)){ev.ok=true;gainExp(O,'int',8);const d=hurt(B,t,ubase(B,u)*(0.5+u.int/100)*1.3,false);if(!t.dead)t.conf=1;bl(B,`${u.name}對${t.name}施${nm}成功，殲敵 ${fmt(d)}，敵隊陷入混亂`,'hit');}
 ...
 else if(cmd==='siege'){ev.gate=tid;gainExp(O,'lea',6);const w=Math.round(ubase(B,u)*0.3);B.gates[tid]=Math.max(0,B.gates[tid]-w);ev.wall=w;
...
function endDay(B){['a','d'].forEach(s=>{B.morale[s]=Math.round(clamp(B.morale[s]-B.loss[s]/B.start[s]*100,0,100));});B.day++;checkMap(B);}
...
function aiPlan(B,u){
 ...
 const in2=foes.filter(f=>hdist(p,f.pos)<=2).sort((a,b)=>a.troops-b.troops);
 const st=foes.filter(f=>hdist(p,f.pos)<=3&&stratChance(u,f)>=0.55).sort((a,b)=>b.troops-a.troops)[0];
 const du=u.off!=null&&u.war>=85?adj.find(f=>f.off!=null&&f.war<u.war-5):null;
 let cmd='wait',tid=null;
 if(du&&Math.random()<0.4){cmd='duel';tid=du.id;}
